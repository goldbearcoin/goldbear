...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::Goldbear)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("Goldbear")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeGoldbearAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeGoldbearAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeGoldbearAmount = strAnonymizeGoldbearAmount.remove(strAnonymizeGoldbearAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeGoldbearAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeGoldbearAmount * COIN) nMaxToAnonymize = nAnonymizeGoldbearAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeGoldbearAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeGoldbearAmount));
        strAnonymizeGoldbearAmount = strAnonymizeGoldbearAmount.remove(strAnonymizeGoldbearAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeGoldbearAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
